...
string rr(string s, int n);
string rl(string s, int n);
string c(string s, int n);
string j(string s, int n);
...
void printS(string s) {
    ...
    for(int i = start; i != end; i += increment) {
        for(int j = 0; j < indexOriginal[i].size(); j++) {
            if(raw[indexOriginal[i][j]].find(s) != string::npos) {
                cout << justified[i] << endl;
                break;
            }
        }
